    ...
    */
    const validateCollectionSlug = async (slug: string) => {
        try {
            ...
            const collection = await fetcher(apiRoutes.getCollection.replace(":slug", slug))
            if (collection.data) {
    ...
    const handleSingleFileChange = (file: Blob, fileResultHandler: (update: ArrayBuffer | string | null) => void ) => {
            ...
            <div className="flex flex-col md:flex-row gap-4 justify-center">
                <div className="w-full">
                    ...
                    <div className="my-2">
                        ...
                        <p className="my-4">
                            {
                                collectionData && collectionData.tags?.length > 0
                                &&
                                splitAtWhiteSpaceOrComma(collectionData.tags)
                                .map((tag, index) =>
                                    <Tag 
                                        key={tag+index}
                                        text={tag} 
                            ...
                            <FileDropzone
                                ...
                                onChange={(event) => {
                                    if (event.target.files?.length) {
                                        handleSingleFileChange(event.target.files[0], setImageFile)
                                    }
                                }}
                            />
                        </div>
                    ...
                    <div>
                        ...
                        <div className={!!bannerFile ? "hidden" : ""}>
                            <FileDropzone
                                ...
                                onChange={(event) => {
                                    if (event.target.files?.length) {
                                        handleSingleFileChange(event.target.files[0], setBannerFile)
                                    }
                                }}
                            />
                        </div>
